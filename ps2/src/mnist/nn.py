...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # *** START CODE HERE ***
    params = {}
    params['W1'] = np.random.normal(0, 1, (input_size, num_hidden))
    params['b1'] = np.zeros(num_hidden)
    params['W2'] = np.random.normal(0, 1, (num_hidden, num_output))
...
def forward_prop(data, one_hot_labels, params):
    ...
    y = softmax(np.dot(a, params['W2']) + params['b2'])
    loss = -np.sum(one_hot_labels * np.log(y)) / data.shape[0]
    return a, y, loss
    # *** END CODE HERE ***
...
def backward_prop(data, one_hot_labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    a, y, loss = forward_prop_func(data, one_hot_labels, params)
    grad = {}
    grad['W2'] = np.dot(a.T, y - one_hot_labels) / data.shape[0]
    grad['b2'] = np.sum(y - one_hot_labels, axis=0) / data.shape[0]
    grad['W1'] = np.dot(data.T, np.dot(y - one_hot_labels, params['W2'].T) * a * (1 - a)) / data.shape[0]
    grad['b1'] = np.sum(np.dot(y - one_hot_labels, params['W2'].T) * a * (1 - a), axis=0) / data.shape[0]
    return grad
    # *** END CODE HERE ***
...
def backward_prop_regularized(data, one_hot_labels, params, forward_prop_func, reg):
    ...
    """
    # *** START CODE HERE ***
    grad = backward_prop(data, one_hot_labels, params, forward_prop_func)
    grad['W2'] += 2 * reg * params['W2']
    grad['W1'] += 2 * reg * params['W1']
    return grad
    # *** END CODE HERE ***
...
def gradient_descent_epoch(train_data, one_hot_train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for i in range(0, train_data.shape[0], batch_size):
        grad = backward_prop_func(train_data[i:i+batch_size], one_hot_train_labels[i:i+batch_size], params, forward_prop_func)
